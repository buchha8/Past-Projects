...
void setup()
{
  ...
  //PWM pins for four pairs of MOSFETs
   pinMode(AD_FET,OUTPUT);
   pinMode(BC_FET,OUTPUT);
   pinMode(EH_FET,OUTPUT);
   pinMode(FG_FET,OUTPUT);
...
void forward_drive()
{
  analogWrite(AD_FET, motor_speed);
  analogWrite(BC_FET, 0);
  analogWrite(EH_FET, 255 - motor_speed);
  analogWrite(FG_FET, 0);
}

void reverse_drive()
{
  analogWrite(AD_FET, 0);
  analogWrite(BC_FET, motor_speed);
  analogWrite(EH_FET, 0);
  analogWrite(FG_FET, 255 - motor_speed);
}
...
void set_robot_status()
{
  motor_speed = N64_status.stick_x;
  
  if((N64_status.data1 & 0x80) != 0)
  {
      forward_drive();
  }
  else if((N64_status.data1 & 0x40) != 0)
  {
      reverse_drive();
  }
  else
...
void loop()
{
   ...
   for (i=0; i<16; i++) {
    ...
    //Takes N64_status and uses it to control robot
    set_robot_status();
